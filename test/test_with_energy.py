...
def calculate_energy():
    ...
    return total_energy, np.mean(distance)


test = np.array([[19, 5, 1, 9, 1, 8, 0, 5.5],
                [2, 8, 14, 1, 12, 7, 0.1, 8.9],
                [5, 6.2, 16, 1.7, 12.9, 7.1, 13.7, 8]])
...
for move in range(1, num_moves+1):
    ...
    old_energy, old_distance = calculate_energy()
    energies = np.append(energies, old_energy)
    distances = np.append(distances, old_distance)
    ...
    new_energy, new_distance = calculate_energy()
    ...
    if new_energy <= old_energy:
        ...
        if x >= random.uniform(0.0,1.0):
            ...
            energies = np.append(energies, total_energy)
            distances = np.append(distances, new_distance)
        else:
