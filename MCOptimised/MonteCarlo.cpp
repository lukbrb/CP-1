...
const double max_translate = 0.3;  // angstroms
...
void print_energy(double energy, int move)
{
    char filename[128];

    snprintf(filename, 128, "energie.dat");
...
int main(int argc, const char **argv)
{
    ...
    for (int move=1; move<=num_moves; move = move + 1)
    {
        ...
        if (move % 1000 == 0)
        {
            print_energy(total_energy, move);
        }
